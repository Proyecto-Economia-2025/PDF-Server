...
{
    public class CoreRequestValidator : IRequestLogger
    {

        private readonly IKafkaProducerService _kafkaProducer;
        private readonly string _logTopic;

        public CoreRequestValidator(IKafkaProducerService kafkaProducer, IConfiguration configuration)
        {
        ...
        public async void LogRequest(BaseRequest request, bool isValid, string reason, string flow)
        {
            ...
            try
            {
                ...
                await _kafkaProducer.ProduceAsync(_logTopic, logEntry);
            }
            catch (Exception ex)
            {
